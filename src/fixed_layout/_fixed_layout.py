...
def get_hspaces_inch(
    ignore_labels: tuple[bool, ...], bboxes: NDArray, tbboxes: NDArray
):
    nspaces = len(ignore_labels)
    hpads_inch = np.empty(nspaces, dtype=float)

    first_bboxes = select_bboxes(ignore_labels[0], bboxes, tbboxes)[:, 0]
    hpads_inch[0] = np.amin([t.x0 for t in first_bboxes])

    for i in range(1, nspaces - 1):
        left_bboxes = select_bboxes(ignore_labels[i], bboxes, tbboxes)[:, i]
        right_bboxes = select_bboxes(ignore_labels[i], bboxes, tbboxes)[:, i - 1]
        left = np.amin([t.x0 for t in left_bboxes])
        right = np.amax([t.x1 for t in right_bboxes])
        hpads_inch[i] = left - right

    last_bboxes = select_bboxes(ignore_labels[-1], bboxes, tbboxes)[:, -1]
    hpads_inch[-1] = np.amin([t.x1 for t in last_bboxes])
...
def get_vspaces_inch(
    ...
    first_bboxes = select_bboxes(ignore_labels[0], bboxes, tbboxes)[0]
    vpads_inch[0] = np.amin([t.y1 for t in first_bboxes])

    for i in range(1, nspaces - 1):
        top_bboxes = select_bboxes(ignore_labels[i], bboxes, tbboxes)[i]
        bottom_bboxes = select_bboxes(ignore_labels[i], bboxes, tbboxes)[i - 1]
        top = np.amin([t.y1 for t in top_bboxes])
        bottom = np.amax([t.y0 for t in bottom_bboxes])
...
def append_colorbars(fig, axes, bboxes_inch) -> tuple[NDArray, NDArray]:
    caxes = []
    caxes_bboxes_inch = []
    for ax in axes.flat:
        for cbar in ax._colorbars:
            caxes.append(cbar.ax)
            caxes_bboxes_inch.append(get_ax_bbox_inch(fig, cbar.ax))
    all_axes = np.append(axes.flat, caxes)
    all_bboxes = np.append(bboxes_inch.flat, caxes_bboxes_inch)
    return all_axes, all_bboxes


def add_margins_inch(
    fig: Figure, margins: utils.Quadrants, axes: NDArray, bboxes_inch: NDArray
):
    fw_old, fh_old = fig.get_size_inches()
    fw_new = fw_old + margins.left + margins.right
    fh_new = fh_old + margins.top + margins.bottom

    all_axes, all_bboxes = append_colorbars(fig, axes, bboxes_inch)

    fig.set_size_inches(fw_new, fh_new)

    for ax, bbox in zip(all_axes, all_bboxes):
        bbox_new = Bbox.from_bounds(
            bbox.x0 + margins.left, bbox.y0 + margins.bottom, bbox.width, bbox.height
        )
        set_axes_position_inch(bbox_new, ax, fig)


def get_cbar_axes(axes) -> list[list[list[Axes]]]:
    return [[ax._colorbars for ax in axes_] for axes_ in axes]


def get_cbar_axes_bboxes(fig, cbar_axes) -> list[list[list[Bbox]]]:
    return [
...
def add_hspace_inch(
    ...
    fig.set_size_inches(fw_old, fh_new, forward=False)
...
def add_vspace_inch(
    ...
    fw_old, fh_old = fig.get_size_inches()
    fw_new = fw_old + vspace
    fig.set_size_inches(fw_new, fh_old, forward=False)

    for (row, col), ba in np.ndenumerate(axes_bboxes_inch):
        shift = 0.0 if col < idx else vspace
...
def do_fixed_layout(fig: Figure, **params):
    ...
    caxs = get_cbar_axes(axes_grid)
    caxs_bboxes = get_cbar_axes_bboxes(fig, caxs)
    bboxes = get_bboxes_inch_grid(fig, axes_grid)

    for _ in range(2):
        tbboxes = get_tbboxes_inch_grid(fig, axes_grid, renderer)

        hpads_now = get_hspaces_inch(nparams["hpads_use_bbox"], bboxes, tbboxes)
        vpads_now = get_vspaces_inch(nparams["vpads_use_bbox"], bboxes, tbboxes)
